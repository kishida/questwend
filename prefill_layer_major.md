...
- **次の伸びしろ**: layer-major では層あたり和集合 ≈ 255/256 なので、0.6MB×255 のバラ読みを
  「層テンソルまるごとの連続読み」に合体できる(ceil(union/some) 回の大きな pread)。
  シーケンシャルなら ~2GB/s 級 → prefill fetch をさらに ~3倍縮められる見込み。
